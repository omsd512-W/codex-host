...
export class DeepSeekV013ProtocolError extends ModernHistoryError {
...
function validateV013Chunk(value: Readonly<Record<string, unknown>>): void {
  switch (value.type) {
    ...
    case "finish":
      ...
      validateChunk(value, validateV013Content);
      return;
    default:
...
export const DEEPSEEK_V013_PROFILE = Object.freeze<DeepSeekModernProfile>({
  version: "0.1.3-rc.1",
  checkpointPrefix: "v2-turn-end:",
  matchesForkTail,
  sessionFormatVersion: 2,
  assistantStream: true,
  snapshotKeys: V013_JOURNAL_SNAPSHOT_KEYS,
  parseHeader: parseV013JournalHeader,
  parseHistoryRecord: (value, remaining) => parseV013HistoryRecord(value, remaining, parseEvent),
  parseLiveItem: (value) => parseV013LiveItem(value, parseEvent),
  parseAssistantBaseline: parseV013AssistantBaseline,
  inheritedEventCount: (header, events) =>
    v013InheritedEventCount(header.isSeeded === true, events),
  validateContent: validateV013Content,
  validateChunk: (value) => {
    if (!isRecord(value)) throw invalid("assistant stream chunk");
    validateV013Chunk(value);
  },
  validateEvent(event) {
    ...
    switch (event.type) {
      ...
      case "assistant/message":
        ...
        expandV013AssistantStream(data.stream);
        break;
      ...
      case "session/end-seed":
        ...
        if (Object.hasOwn(data, "inherited") && data.inherited !== true)
          fail("DSH v0.1.3 inherited marker is malformed");
        break;
  ...
  settlementUsage(data) {
    ...
    let usage: unknown;
    for (const { chunk } of expandV013AssistantStream(data.stream)) {
